...
async function seed() {
  ...
  const mockDataPath = path.join(__dirname, '../packages/web/src/data/mockVenues.ts');
  const mockDataContent = fs.readFileSync(mockDataPath, 'utf8');

  // Simple regex to extract venue objects from the TS file
  const venueRegex = /\{[\s\S]*?id: '(.*?)',[\s\S]*?name: '(.*?)',[\s\S]*?location: '(.*?)',[\s\S]*?capacity: (.*?),[\s\S]*?imageUrl: '(.*?)',[\s\S]*?pricePerHour: (.*?),[\s\S]*?\}/g;
  ...
  for (const venue of venues) {
    ...
    // 1. Get upload URL
    const uploadUrlResp = await fetch(`${API_URL}/images/upload-url`, { method: 'POST' });
    if (!uploadUrlResp.ok) {
      throw new Error(`Failed to get upload URL: ${uploadUrlResp.statusText}. Ensure your CF_IMAGES_API_TOKEN is set in .dev.vars`);
    }
    ...
    try {
      ...
      if (!imgResp.ok) {
        throw new Error(`Failed to download image from Unsplash: ${imgResp.statusText}`);
      }
      const arrayBuffer = await imgResp.arrayBuffer();
      const buffer = Buffer.from(arrayBuffer);

      const formData = new FormData();
      formData.append('file', new Blob([buffer], { type: 'image/jpeg' }), 'venue.jpg');
      ...
      if (!cfResp.ok) {
        ...
        throw new Error(`Failed to upload real image to Cloudflare: ${cfResp.statusText}. Body: ${text}`);
      }
      console.log(`  Successfully uploaded image for ${venue.name}`);
    } catch (e) {
      console.error(`  Warning: Error during image processing for ${venue.name}: ${e.message}`);
